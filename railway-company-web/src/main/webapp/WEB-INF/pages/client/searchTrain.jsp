    ...
    <div id="inputtabs" class="tab-panel">
        ...
        <c:if test="${!empty foundTrains}">
            <div class="wide-form-box">
                <h2 class="title"><spring:message code="route.between.label"/>
                    <c:choose>
                        <c:when test="${pageContext.response.locale == 'ru'}">
                            ${fromStation.ruName} - ${toStation.ruName}
                        </c:when>
                        <c:otherwise>
                ...
                <table class="simple-little-table" cellspacing='0'>
                    <tr>
                        ...
                        <td><p><spring:message code="price.label"/></p></td>
                    ...
                    <c:forEach var="train" items="${foundTrains}">
                        <c:forEach var="route" items="${train.routes}" varStatus="status">
                            <c:if test="${status.first}">
                                <c:choose>
                                    <c:when test="${pageContext.response.locale == 'ru'}">
                                        <c:set var="firstStation" scope="request"
                            ...
                            <c:if test="${status.last}">
                                <c:choose>
                                    <c:when test="${pageContext.response.locale == 'ru'}">
                                        <c:set var="lastStation" scope="request"
                                               value="${route.arrivalStation.ruName}"/>
                                    </c:when>
                                    <c:otherwise>
                                        <c:set var="lastStation" scope="request" value="${route.arrivalStation.name}"/>
                                    </c:otherwise>
                                </c:choose>
                            </c:if>
                            <c:if test="${route.departureStation eq fromStation}">
                                <c:set var="calc" scope="request" value="true"/>
                                <c:set var="departureDate" scope="request" value="${route.departureDate}"/>
                            </c:if>
                            <c:if test="${calc}">
                                <c:set var="price" scope="request" value="${route.cost + price}"/>
                            </c:if>
                            <c:if test="${route.arrivalStation eq toStation}">
                                <c:set var="arrivalDate" scope="request" value="${route.arrivalDate}"/>
                                <c:set var="calc" scope="request" value="false"/>
                            </c:if>
                        </c:forEach>
                        <tr>
                            <td>
                            ...
                            <td>${price}USD</td>
                            <td>
                                <form action="<c:url value='/client/buyTicket'/>" method="GET">
                                    <input type="hidden" name="trainId" value="${train.id}">
                                    <input type="hidden" name="startStationId" value="${fromStation.id}">
                                    <input type="hidden" name="endStationId" value="${toStation.id}">
